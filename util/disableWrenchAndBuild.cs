			...
			for (%i = 0; %i < getFieldCount(%data); %i++)
			{
				...
				switch$ (%type)
				{
					...
					case "VDB":
						...
						{
							if (%cl.wrenchBrick.getGroup().bl_id != %cl.bl_id && isObject(getWord(%field, 1)))
							{
								%data = setField(%data, %i, "VDB 0");
								if (isObject(%cl.wrenchBrick.vehicle))
								{
									purchaseVehicle(%cl, 0);
								}
								messageClient(%cl, '', "You can only buy vehicles on your own vehicle spawn!");
								continue;
							}
	...
	function serverCmdPlantBrick(%cl)
	{
		if (isObject(%pl = %cl.player) && isObject(%pl.tempBrick) && !%cl.isBuilder)
		{
			...
			if ((%db.category $= "Baseplates" && %db.subCategory !$= "Plain") || %db.subCategory $= "Drinks" || %db.subCategory $= "Holes"
				...
				|| %db.uiName $= "Waving Flag" || %db.uiName $= "Still Flag" || %db.uiName $= "")
			{
